...
```{r}
# Cargamos las dos librerias necesarias antes de todo:
library(MASS)
library(robustbase)

# Una vez tenemos las dos librerías pasamos a cada conjunto de datos.

# Primero miramos el conjunto de datos mammals del paquete MASS
# a) Miramos las características. 
df_mammals <- mammals
summarise(df_mammals)
# b) Usamos las funciones siguientes para una primera visión de los conjuntos de 
# datos.
dim(df_mammals)
head(df_mammals)
head(df_mammals, n=3)
tail(df_mammals)
str(df_mammals)
# c1) Mostramos los nombres de las filas.
rownames(df_mammals)
# c2) Mostramos los nombres de las columnas.
colnames(df_mammals)

# Segundo miramos el conjunto de datos Animals2 del paquete robustbase
# a) Miramos las características. 
df_animals <- Animals2
...
```{r}
# Tenemos los animales que no estan en mammals
diferentes_mammals <- setdiff(rownames(df_mammals), rownames(df_animals))
diferentes_mammals
# Tenemos los animales que no estan en animals
diferentes_animals <- setdiff(rownames(df_animals), rownames(df_mammals))
diferentes_animals
```
...
```{r}
diferentes_m <- setdiff(rownames(df_mammals), rownames(df_animals))
diferentes_m
```
...
```{r}
# Primero hacemos la función del qqPlot de los mammals
qqPlot(log(mammals$brain))
# Segundo hacemos la función del qqPlot de los animals
...
```{r}
# a) Asignamos el valor devuelto por la funcion library.
libReturn <- library()
libReturn
# b) Mostramos por pantalla los 5 primeros elementos de la matriz
head(libReturn$results, n=5)
# también podemos observar los 5 primeros elementos de la matriz con la siguiente
# función:
libReturn[[2]] [1:5]
# c) Determinamos el número de librerias que tenemos instaladas
length(libReturn)
```
...
```{r}
## 1º cabbages del paquete MASS
# a) Determinamos las tres etapas para el conjunto de datos cabbages.
head(cabbages)
dim(cabbages)
tail(cabbages)
glimpse(cabbages)
str(cabbages)
# b) Determinamos el número de valores perdidos para cada una de las variables
# del conjunto cabbages.
any(is.na(cabbages))
colSums(sapply(cabbages, is.na))

## 2º Chile del paquete car
# a) Determinamos las tres etapas para el conjunto de datos Chile.
head(Chile)
dim(Chile)
tail(Chile)
glimpse(Chile)
...
# d) Utilizamos la función summary para que nos saque los estadísticos y el número
# de valores perdidos de cada variable.
summary(cabbages)
summary(Chile)
```
...
```{r}
# Con estas dos funciones vamos a ver a traves de la aproximación visual, si nuestras
# variables HeadWt y VitC del conjunto cabbages verifican que siguen una distribución
# Gaussiana.
plot_hw <- qqPlot(cabbages$HeadWt)
plot_hw
plot_vc <- qqPlot(cabbages$VitC)
plot_vc
```
...
```{r}
# Utilizamos un histograma para determinar los datos de nuestras variables cuantitativas.
repre <- ggplot(cabbages, aes(VitC,fill = Cult)) + geom_histogram(alpha=0.5)
repre
```
...
```{r}
# a) Para ver lo que nos piden vamos a hacer un diagrama de dispersión global.
plot(cabbages$HeadWt, cabbages$VitC)

# b) Ahora vamos a hacer un diagrama de dispersión, pero este lo vamos a etiquetar 
# según un factor concreto.
plot(cabbages$HeadWt, cabbages$VitC, pch=c(6,16)[cabbages$HeadWt])
# Planteamos 2 modelos lineales, cada uno para los datos de un factor.
Model1 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == 'c39'))
Model2 <- lm(VitC ~ HeadWt, data = cabbages, subset = which(Cult == 'c52'))
# Ahora vamos a representar las rectas correspondientes a cada modelo lineal.
abline(Model1, lty=2)
abline(Model2)
...
```{r}
# Vamos a observar los coeficientes del modelo 1:
summary(Model1$coefficients)
# Vamos a observar los coeficientes del modelo 2:
summary(Model2$coefficients)

# Observamos la pendiente del modelo 1:
lm(Model1)
# Obtenemos que es negativa

# Observamos la pendiente del modelo 2:
lm(Model2)
# Obtenemos que es negativa
```
